[**YooKassa SDK API Reference**](../../../README.md)

***

[YooKassa SDK API Reference](../../../README.md) / Payments

# Payments

Все, что касается платежей в ЮКассе

## Interfaces

- [CapturePaymentRequest](interfaces/CapturePaymentRequest.md)
- [IPayment](interfaces/IPayment.md)
- [IRecipient](interfaces/IRecipient.md)
- [PaymentCancellationDetails](interfaces/PaymentCancellationDetails.md)
- [Statement](interfaces/Statement.md)
- [StatementDeliveryMethod](interfaces/StatementDeliveryMethod.md)

## Type Aliases

- [AuthorizationDetails](type-aliases/AuthorizationDetails.md)
- [CancelReason](type-aliases/CancelReason.md)
- [CreatePaymentRequest](type-aliases/CreatePaymentRequest.md)
- [DealType](type-aliases/DealType.md)
- [PaymentStatus](type-aliases/PaymentStatus.md)
- [StatementType](type-aliases/StatementType.md)
- [TransferPayment](type-aliases/TransferPayment.md)
