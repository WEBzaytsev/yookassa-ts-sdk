...
SDK валидирует пользовательские ключи идемпотентности перед отправкой запроса. Если ключ превышает **64 символа**, клиент выбрасывает `YooKassaErr` с `code: 'invalid_request'` ещё до сетевого вызова. Автоматически сгенерированные ключи (когда ключ не передан) всегда занимают 32 символа и безопасны.

```ts
// Выбросит ошибку немедленно — ключ длиной 65 символов
await sdk.payments.create(data, 'a'.repeat(65))

// Безопасно — используется автоматически сгенерированный ключ
