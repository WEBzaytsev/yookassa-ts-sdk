...
[Безопасная сделка](https://yookassa.ru/developers/solutions-for-platforms/safe-deal/basics) позволяет маркетплейсу удерживать платежи покупателей и выплачивать продавцам после закрытия сделки. SDK предоставляет управление сделками через `sdk.deals.*`; платежи, возвраты и выплаты привязываются к сделке через `deal: { id }`.
...
При закрытии сделки ЮKassa отправляет событие `deal.closed`. См. [Вебхуки](webhooks.md) для обработки входящих уведомлений.
